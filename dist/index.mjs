...
function buildJs() {
  ...
  js.push("  if (!currentMatchMsgs || currentMatchMsgs.length === 0) { nav.style.display = 'none'; if (cnt) cnt.textContent = '0/0'; return; }");
  js.push("  nav.style.display = 'inline-flex';");
  js.push("  if (cnt) cnt.textContent = (currentMatchIdx >= 0 ? currentMatchIdx + 1 : 0) + '/' + currentMatchMsgs.length;");
  js.push("}");
  js.push("function gotoMatch(idx){");
  js.push("  if (!currentMatchMsgs || currentMatchMsgs.length === 0) return;");
  ...
  js.push("  var bt = document.getElementById('btnThinking'); if (bt) bt.addEventListener('click', function(){ showThinking = !showThinking; updateVisibility(); if (isSearchActive) doMsgSearch(); });");
  js.push("  var btf = document.getElementById('btnThinkingFab'); if (btf) btf.addEventListener('click', function(){ showThinking = !showThinking; updateVisibility(); if (isSearchActive) doMsgSearch(); });");
  js.push("  var bf = document.getElementById('btnTools'); if (bf) bf.addEventListener('click', function(){ showTools = !showTools; updateVisibility(); if (isSearchActive) doMsgSearch(); });");
  js.push("  var bff = document.getElementById('btnToolsFab'); if (bff) bff.addEventListener('click', function(){ showTools = !showTools; updateVisibility(); if (isSearchActive) doMsgSearch(); });");
  ...
  js.push("    else if (e.key === 'ArrowUp') { if (isSearchActive && currentMatchMsgs && currentMatchMsgs.length) { e.preventDefault(); gotoMatch(currentMatchIdx - 1); } }");
  js.push("  });");
...
function buildListHtml(state) {
  ...
  lines.push('      <div class="msg-search"><div class="search-wrap"><input type="search" id="msgSearch" placeholder="\u641C\u7D22\u6D88\u606F\u5173\u952E\u5B57\u2026"><button type="button" class="search-inside-clear" id="searchInputClear" aria-label="\u6E05\u9664\u641C\u7D22" style="display:none">\xD7</button></div><span class="search-nav" id="searchNav" style="display:none"><button type="button" class="nav-btn" id="searchPrev" aria-label="\u4E0A\u4E00\u4E2A\u5339\u914D" title="\u4E0A\u4E00\u4E2A\u5339\u914D">\u25B2</button><span class="nav-count" id="searchNavCount">0/0</span><button type="button" class="nav-btn" id="searchNext" aria-label="\u4E0B\u4E00\u4E2A\u5339\u914D" title="\u4E0B\u4E00\u4E2A\u5339\u914D">\u25BC</button></span></div>');
  lines.push("    </div>");
