...
function buildJs() {
  ...
  js.push("    var trunc = res.headers && res.headers.get && res.headers.get('X-Export-Truncated') === '1';");
  ...
  js.push("    b.textContent = '已复制 ' + bytes + ' 字' + (trunc ? '（已截断）' : '');");
...
// byteBudget: 超过该字节数则截断导出（仅保留开头部分），避免超大会话把
// 服务端 / 浏览器内存打爆或剪贴板写入卡死。默认 Infinity = 不限制（详情页下载用）。
export function buildExportText(session, messages, byteBudget = Infinity) {
  const lines = [];
  let bytes = 0;
  let truncated = false;
  const push = function (s) {
    const b = Buffer.byteLength(s, "utf8");
    if (byteBudget !== Infinity && bytes + b > byteBudget) {
      truncated = true;
      return false;
    }
    lines.push(s);
  ...
  push("会话：" + (session.display_name || session.sender_name || session.session_id || ""));
  push("Agent：" + (session.agent_id || ""));
  push("来源：" + sourceLabelTs(session.channel) + "  分类：" + catLabelTs(session));
  push("");
  messages.forEach(function (m) {
    ...
    if (!push(who + " " + fmtTimeShort(m.timestamp))) return;
    push(plainTextTs(m));
    push("");
  });
  if (truncated) {
    lines.push("");
    lines.push("【提示】本会话内容过长，已自动截断导出（仅包含开头部分，约 " + Math.round(byteBudget / 1024 / 1024) + "MB）。完整记录请在「会话详情」中分页查看，或缩小筛选范围后重试。");
  }
