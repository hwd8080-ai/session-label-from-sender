...
function buildListHtml(state) {
  ...
  lines.push("      <div class=\"drawer-tools\"><button class=\"toggle-btn active\" id=\"btnThinking\" aria-pressed=\"true\">🧠 思考</button><button class=\"toggle-btn active\" id=\"btnTools\" aria-pressed=\"true\">🔧 工具</button></div>");
...
function renderMessagesHtml(msgs) {
  ...
  msgs.forEach(function (m) { html += renderMessageHtml(m); });
