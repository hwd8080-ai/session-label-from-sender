...
// ../../.workbuddy/binaries/node/workspace/node_modules/punycode.js/punycode.js
var require_punycode = __commonJS({
  "../../.workbuddy/binaries/node/workspace/node_modules/punycode.js/punycode.js"(exports, module) {
    "use strict";
...
// ../../.workbuddy/binaries/node/workspace/node_modules/mdurl/index.mjs
var mdurl_exports = {};
...
// ../../.workbuddy/binaries/node/workspace/node_modules/mdurl/lib/decode.mjs
var decodeCache = {};
...
// ../../.workbuddy/binaries/node/workspace/node_modules/mdurl/lib/encode.mjs
var encodeCache = {};
...
// ../../.workbuddy/binaries/node/workspace/node_modules/mdurl/lib/format.mjs
function format(url) {
  let result = "";
  result += url.protocol || "";
  result += url.slashes ? "//" : "";
...
// ../../.workbuddy/binaries/node/workspace/node_modules/mdurl/lib/parse.mjs
function Url() {
  this.protocol = null;
  this.slashes = null;
  this.auth = null;
...
// ../../.workbuddy/binaries/node/workspace/node_modules/uc.micro/build/index.mjs
var build_exports = {};
...
// ../../.workbuddy/binaries/node/workspace/node_modules/entities/dist/decode-codepoint.js
...
// ../../.workbuddy/binaries/node/workspace/node_modules/entities/dist/internal/decode-shared.js
...
// ../../.workbuddy/binaries/node/workspace/node_modules/entities/dist/generated/decode-data-html.js
...
// ../../.workbuddy/binaries/node/workspace/node_modules/entities/dist/internal/bin-trie-flags.js
var BinTrieFlags;
...
// ../../.workbuddy/binaries/node/workspace/node_modules/entities/dist/decode.js
var CharCodes;
(function(CharCodes2) {
...
// ../../.workbuddy/binaries/node/workspace/node_modules/entities/dist/index.js
var EntityLevel;
...
// ../../.workbuddy/binaries/node/workspace/node_modules/linkify-it/build/index.mjs
var REBuilder = class {
  src_Any = Any.source;
  src_Cc = Cc.source;
  src_Z = Z.source;
...
// ../../.workbuddy/binaries/node/workspace/node_modules/markdown-it/dist/markdown-it.mjs
...
var mdServer = MarkdownItCallable({ html: false, linkify: true, breaks: true, typographer: false });
var _mdClientSrc = null;
function mdClientSrc() {
  if (_mdClientSrc !== null) return _mdClientSrc;
  const cands = [
    path3.join(__dirname, "dist", "md-client.js"),
    path3.join(__dirname, "md-client.js"),
    path3.join(__dirname, "..", "dist", "md-client.js")
  ];
  for (const c of cands) {
    try {
      _mdClientSrc = fs3.readFileSync(c, "utf8");
      return _mdClientSrc;
    } catch {
    }
  }
...
function buildJs() {
  ...
  js.push(`      if (b.type === 'thinking') { out += '<details class="msg-collapse" data-type="thinking"><summary>\u{1F4AD} \u601D\u8003\u8FC7\u7A0B</summary><div class="thinking">' + esc(b.thinking || '') + '</div></details>'; return; }`);
  js.push(`      if (b.type === 'toolCall' || b.type === 'tool_use') { out += '<details class="msg-collapse" data-type="tool"><summary>\u{1F527} \u8C03\u7528\u5DE5\u5177\uFF1A' + esc(b.name || 'tool') + '</summary>' + toolCallCard(b) + '</details>'; return; }`);
  ...
  js.push("      var out = '';");
  js.push("      c.forEach(function(b){");
  js.push(`        if (b && b.type === 'thinking') out += '<details class="msg-collapse" data-type="thinking"><summary>\u{1F4AD} \u601D\u8003\u8FC7\u7A0B</summary><div class="thinking">' + esc(b.thinking || '') + '</div></details>';`);
  js.push(`        else if (b && (b.type === 'toolCall' || b.type === 'tool_use')) out += '<div class="actool"><details><summary>\u{1F527} ' + esc(b.name || 'tool') + '</summary>' + toolCallCard(b) + '</details></div>';`);
  js.push(`        else if (typeof b === 'string') out += '<div class="actres"><details><summary>\u{1F4E6} ' + (isErr ? '\u26A0 ' : '') + esc(m.tool_name || '\u5DE5\u5177\u7ED3\u679C') + '</summary><pre>' + esc(b.substring(0,4000)) + '</pre></details></div>';`);
  js.push(`        else if (b && b.type === 'text') out += '<div class="actres"><details><summary>\u{1F4E6} ' + (isErr ? '\u26A0 ' : '') + esc(m.tool_name || '\u5DE5\u5177\u7ED3\u679C') + '</summary><pre>' + esc((b.text||'').substring(0,4000)) + '</pre></details></div>';`);
  js.push("      });");
  js.push("      return out;");
  js.push("    }");
  js.push("    var text = (typeof c === 'string') ? c : (c ? JSON.stringify(c, null, 2) : '');");
  js.push(`    return '<div class="actres' + (isErr ? ' err' : '') + '"><details><summary>\u{1F4E6} ' + (isErr ? '\u26A0 ' : '') + esc(m.tool_name || '\u5DE5\u5177\u7ED3\u679C') + '</summary>' + (text ? '<pre>' + esc(text.substring(0, 4000)) + '</pre>' : '') + '</details></div>';`);
  js.push("  }");
  ...
  js.push(`  var head = '<summary class="ahead"><span class="achev">\u25B8</span><span class="aico">\u{1F527}</span><span class="atitle">Activity</span><span class="acount">' + n + ' \u4E2A\u5DE5\u5177</span></summary>';`);
  ...
  js.push("  isSearchActive = false; msgHighlight = null; currentMatchIdx = -1; currentMarks = [];");
  ...
  js.push("  // starts from a clean slate (no stale offset/seq leaking into next openDrawer).");
  js.push("  msgOffset = 0; msgAllLoaded = false; oldestSeq = null; newestSeq = null; msgMode = 'desc';");
  js.push("  currentMessages = []; topDate = null; bottomDate = null; msgHighlight = null;");
  js.push("  updateSearchClearButton(); updateDateClearButtons();");
  js.push("  var fg = $('fabGroup'); if (fg) { fg.classList.remove('open'); }");
  js.push("  var ft = $('fabToggle'); if (ft) { ft.setAttribute('aria-expanded', 'false'); ft.setAttribute('aria-label', '\u5C55\u5F00\u5FEB\u6377\u64CD\u4F5C'); }");
  ...
  js.push("  });");
  js.push("}");
  js.push("function updateMatchNav(){");
  js.push("  var nav = $('searchNav'); if (!nav) return;");
  js.push("  var cnt = $('searchNavCount');");
  js.push("  if (!currentMarks || currentMarks.length === 0) { nav.style.display = 'none'; if (cnt) cnt.textContent = '0/0'; return; }");
  js.push("  nav.style.display = 'inline-flex';");
  js.push("  if (cnt) cnt.textContent = (currentMatchIdx >= 0 ? currentMatchIdx + 1 : 0) + '/' + currentMarks.length;");
  js.push("}");
  js.push("function gotoMatch(idx){");
  js.push("  if (!currentMarks || currentMarks.length === 0) return;");
  js.push("  var len = currentMarks.length;");
  js.push("  if (idx < 0) idx = len - 1;");
  js.push("  if (idx >= len) idx = 0;");
  js.push("  if (currentMatchIdx >= 0 && currentMarks[currentMatchIdx]) currentMarks[currentMatchIdx].classList.remove('active');");
  js.push("  currentMatchIdx = idx;");
  js.push("  var m = currentMarks[currentMatchIdx];");
  ...
  js.push("  var box = $('messages');");
  js.push("  if (!q) {");
  js.push("    isSearchActive = false; msgHighlight = null; count.textContent = ''; currentMatchIdx = -1; currentMarks = [];");
  ...
  js.push(`    if (msgsArr.length === 0) { box.innerHTML = '<div class=\\"loading\\">\u672A\u627E\u5230\u5339\u914D\u7684\u6D88\u606F</div>'; currentMarks = []; currentMatchIdx = -1; }`);
  ...
  js.push("    currentMarks = Array.prototype.slice.call(box.querySelectorAll('mark'));");
  js.push("    if (currentMarks.length) { currentMatchIdx = -1; gotoMatch(0); } else { currentMatchIdx = -1; updateMatchNav(); }");
  js.push("    updateSearchClearButton();");
  js.push(`  } catch (e) { box.innerHTML = '<div class=\\"loading\\">\u641C\u7D22\u5931\u8D25\uFF1A' + esc(e.message) + '</div>'; count.textContent = ''; currentMarks = []; currentMatchIdx = -1; updateMatchNav(); }`);
  js.push("  updateVisibility();");
  js.push("}");
  ...
  js.push("    if (e.key === 'Enter') { clearTimeout(this._timer); if (isSearchActive && currentMarks && currentMarks.length) { e.preventDefault(); gotoMatch(currentMatchIdx + 1); } else { doMsgSearch(); } }");
  js.push("    else if (e.key === 'ArrowDown') { if (isSearchActive && currentMarks && currentMarks.length) { e.preventDefault(); gotoMatch(currentMatchIdx + 1); } }");
  js.push("    else if (e.key === 'ArrowUp') { if (isSearchActive && currentMarks && currentMarks.length) { e.preventDefault(); gotoMatch(currentMatchIdx - 1); } }");
  js.push("  });");
  js.push("  var spv = document.getElementById('searchPrev'); if (spv) spv.addEventListener('click', function(){ gotoMatch(currentMatchIdx - 1); });");
  js.push("  var snx = document.getElementById('searchNext'); if (snx) snx.addEventListener('click', function(){ gotoMatch(currentMatchIdx + 1); });");
  js.push("});");
...
function buildListHtml(state) {
  ...
  lines.push('      <div class="drawer-dates"><div class="range"><div class="date-wrap"><input id="dStart" class="control" type="date" aria-label="\u5F00\u59CB\u65E5\u671F"><button type="button" class="date-inside-clear" id="dStartClear" aria-label="\u6E05\u9664\u5F00\u59CB\u65E5\u671F" style="display:none">\xD7</button></div><span>\u81F3</span><div class="date-wrap"><input id="dEnd" class="control" type="date" aria-label="\u7ED3\u675F\u65E5\u671F"><button type="button" class="date-inside-clear" id="dEndClear" aria-label="\u6E05\u9664\u7ED3\u675F\u65E5\u671F" style="display:none">\xD7</button></div></div></div>');
  lines.push('      <div class="msg-search"><div class="search-wrap"><input type="search" id="msgSearch" placeholder="\u641C\u7D22\u6D88\u606F\u5173\u952E\u5B57\u2026"><button type="button" class="search-inside-clear" id="searchInputClear" aria-label="\u6E05\u9664\u641C\u7D22" style="display:none">\xD7</button></div><span class="search-count" id="searchCount"></span><span class="search-nav" id="searchNav" style="display:none"><button type="button" class="nav-btn" id="searchPrev" aria-label="\u4E0A\u4E00\u4E2A\u5339\u914D" title="\u4E0A\u4E00\u4E2A\u5339\u914D">\u25B2</button><span class="nav-count" id="searchNavCount">0/0</span><button type="button" class="nav-btn" id="searchNext" aria-label="\u4E0B\u4E00\u4E2A\u5339\u914D" title="\u4E0B\u4E00\u4E2A\u5339\u914D">\u25BC</button></span></div>');
  lines.push("    </div>");
  lines.push('    <div class="fab-group" id="fabGroup" aria-label="\u5FEB\u6377\u64CD\u4F5C">');
  lines.push('      <button type="button" class="fab-item fab-tool above" id="btnThinkingFab" aria-pressed="false" data-tip="\u601D\u8003" style="order:2">\u{1F9E0}</button>');
  lines.push('      <button type="button" class="fab-item fab-tool above" id="btnToolsFab" aria-pressed="false" data-tip="\u5DE5\u5177" style="order:1">\u{1F527}</button>');
  lines.push('      <button type="button" class="fab-item below" id="jumpTopFab" aria-label="\u5230\u6700\u65E9\u7684\u6D88\u606F" data-tip="\u9876\u90E8" style="order:4">\u25B2</button>');
...
function toolResultCardTs(msg, c) {
  let text2 = "";
  if (typeof c === "string") text2 = c;
  else if (Array.isArray(c)) {
    c.forEach(function(b) {
      if (typeof b === "string") text2 += b + "\n";
      else if (b && b.type === "text") text2 += b.text + "\n";
    });
  } else {
    text2 = JSON.stringify(c, null, 2);
  }
  const isErr = msg.is_error === true || msg.is_error === 1;
  return '<div class="toolcard' + (isErr ? " err" : "") + '"><div class="tname">' + (isErr ? "\u26A0 " : "") + escHtml(msg.tool_name || "tool result") + "</div>" + (text2 ? "<pre>" + escHtml(text2.substring(0, 8e3)) + "</pre>" : "") + "</div>";
}
function messageDataTypeTs(role, content_json) {
  ...
  const visible = c.filter(function(b) {
  ...
  if (visible.every((b) => b.type === "toolCall" || b.type === "tool_use")) return "tool";
  if (visible.every((b) => b.type === "thinking")) return "thinking";
  return "";
}
function isEmptyWhenHiddenTs(role, content_json) {
...
function renderContentHtml(cj, msg) {
  ...
  if (Array.isArray(c)) {
    return c.map(function(b) {
      ...
      if (b.type === "thinking") return '<details class="msg-collapse" data-type="thinking"><summary>\u{1F4AD} \u601D\u8003\u8FC7\u7A0B</summary><div class="thinking">' + escHtml(b.thinking || "") + "</div></details>";
      if (b.type === "toolCall" || b.type === "tool_use") return '<details class="msg-collapse" data-type="tool"><summary>\u{1F527} \u8C03\u7528\u5DE5\u5177\uFF1A' + escHtml(b.name || "tool") + "</summary>" + toolCallCardTs(b) + "</details>";
...
function isActivityMsgTs(m) {
  ...
  if (role === "assistant") {
    ...
    if (!Array.isArray(c)) return false;
    const hasText = c.some(function(b) {
      return b && b.type === "text" && (b.text || "").trim() || typeof b === "string" && b.trim();
    });
    if (hasText) return false;
    return c.some(function(b) {
      return b && (b.type === "toolCall" || b.type === "tool_use");
    });
  }
  return false;
}
function countToolsTs(msgs) {
  ...
  (msgs || []).forEach(function(m) {
    ...
    if (m.role === "assistant") {
      const c = parseCjTs(m.content_json);
      if (Array.isArray(c)) c.forEach(function(b) {
        if (b && (b.type === "toolCall" || b.type === "tool_use")) calls++;
      });
    }
  });
  return calls || res;
}
function isToolCallOnlyTs(m) {
...
function firstToolNameTs(m) {
  const c = parseCjTs(m.content_json);
  if (!Array.isArray(c)) return "";
  for (let i = 0; i < c.length; i++) {
    if (c[i] && (c[i].type === "toolCall" || c[i].type === "tool_use")) return c[i].name || "";
  }
  return "";
}
function coalesceToolActivityTs(msgs) {
  const out = [];
  const arr = msgs || [];
  for (let i = 0; i < arr.length; i++) {
    ...
    if (isToolCallOnlyTs(m) && i + 1 < arr.length && (arr[i + 1].role === "tool" || arr[i + 1].role === "toolResult")) {
      ...
      const c = parseCjTs(m.content_json);
      if (Array.isArray(c)) c.forEach(function(b) {
        if (b && (b.type === "toolCall" || b.type === "tool_use")) callBlock = b;
        else if (b && b.type === "thinking") thinkingBlock = b;
      });
      let resultText = "";
      let rc = res.content_json;
      try {
        rc = JSON.parse(rc);
      ...
      else if (Array.isArray(rc)) rc.forEach(function(b) {
        if (typeof b === "string") resultText += b + "\n";
        else if (b && b.type === "text") resultText += (b.text || "") + "\n";
      });
...
function groupActivityTs(msgs) {
  ...
  let cur = null;
  (msgs || []).forEach(function(m) {
    if (isActivityMsgTs(m)) {
      if (!cur) cur = { type: "activity", msgs: [], startTs: m.timestamp };
      cur.msgs.push(m);
      cur.endTs = m.timestamp;
    } else {
      if (cur) {
        items.push(cur);
        cur = null;
  ...
  return items.map(function(it) {
    if (it.type === "activity" && it.msgs.length === 1) return { type: "msg", m: it.msgs[0] };
    return it;
  });
}
function renderActivityMsgTs(m) {
  ...
  if (role === "tool" || role === "toolResult") {
    ...
    if (Array.isArray(c)) {
      let out2 = "";
      c.forEach(function(b) {
        if (b && b.type === "thinking") out2 += '<details class="msg-collapse" data-type="thinking"><summary>\u{1F4AD} \u601D\u8003\u8FC7\u7A0B</summary><div class="thinking">' + escHtml(b.thinking || "") + "</div></details>";
        else if (b && (b.type === "toolCall" || b.type === "tool_use")) out2 += '<div class="actool"><details><summary>\u{1F527} ' + escHtml(b.name || "tool") + "</summary>" + toolCallCardTs(b) + "</details></div>";
        else if (typeof b === "string") out2 += '<div class="actres"><details><summary>\u{1F4E6} ' + (isErr ? "\u26A0 " : "") + escHtml(m.tool_name || "\u5DE5\u5177\u7ED3\u679C") + "</summary><pre>" + escHtml(b.substring(0, 4e3)) + "</pre></details></div>";
        else if (b && b.type === "text") out2 += '<div class="actres"><details><summary>\u{1F4E6} ' + (isErr ? "\u26A0 " : "") + escHtml(m.tool_name || "\u5DE5\u5177\u7ED3\u679C") + "</summary><pre>" + escHtml((b.text || "").substring(0, 4e3)) + "</pre></details></div>";
      });
      return out2;
    }
    let text2 = typeof c === "string" ? c : c ? JSON.stringify(c, null, 2) : "";
    return '<div class="actres' + (isErr ? " err" : "") + '"><details><summary>\u{1F4E6} ' + (isErr ? "\u26A0 " : "") + escHtml(m.tool_name || "\u5DE5\u5177\u7ED3\u679C") + "</summary>" + (text2 ? "<pre>" + escHtml(text2.substring(0, 4e3)) + "</pre>" : "") + "</details></div>";
  }
  const c2 = parseCjTs(m.content_json);
  if (!Array.isArray(c2)) return "";
  let out = "";
  c2.forEach(function(b) {
    if (b && b.type === "thinking") out += '<details class="msg-collapse" data-type="thinking"><summary>\u{1F4AD} \u601D\u8003\u8FC7\u7A0B</summary><div class="thinking">' + escHtml(b.thinking || "") + "</div></details>";
    else if (b && (b.type === "toolCall" || b.type === "tool_use")) out += '<div class="actool"><details><summary>\u{1F527} ' + escHtml(b.name || "tool") + "</summary>" + toolCallCardTs(b) + "</details></div>";
  });
  return out;
}
function renderItemTs(item) {
  ...
  const head = '<summary class="ahead"><span class="achev">\u25B8</span><span class="aico">\u{1F527}</span><span class="atitle">Activity</span><span class="acount">' + n + " \u4E2A\u5DE5\u5177</span></summary>";
  let body = '<div class="abody">';
  item.msgs.forEach(function(m) {
    body += renderActivityMsgTs(m);
  });
  body += "</div>";
...
function renderMessageHtml(m) {
  ...
  if (isTool) body = '<details class="msg-collapse" data-type="tool"><summary>\u{1F527} ' + escHtml(m.tool_name || "\u5DE5\u5177\u8C03\u7528\u7ED3\u679C") + " \xB7 " + fmtTimeShort(m.timestamp) + "</summary>" + body + "</details>";
...
function buildExportText(session, messages, byteBudget, opts) {
  ...
  if (byteBudget && byteBudget > 0) {
    ...
    const bytes = encoder.encode(text2);
    if (bytes.length > byteBudget) {
      let cut = byteBudget;
      while (cut > 0 && bytes[cut] >> 6 === 2) cut--;
      text2 = new TextDecoder().decode(bytes.slice(0, cut)) + "\n\n[\u5BFC\u51FA\u5185\u5BB9\u8FC7\u957F\uFF0C\u5DF2\u622A\u65AD]";
      truncated = true;
    }
  }
...
function resolveExtensionDir() {
  ...
  while (dir !== path4.dirname(dir)) {
    if (fs4.existsSync(path4.join(dir, "openclaw.plugin.json"))) return dir;
    dir = path4.dirname(dir);
  }
  return path4.dirname(fileURLToPath2(import.meta.url));
}
function resolveDbPath() {
  return path4.join(resolveExtensionDir(), "data", "session-admin.db");
}
