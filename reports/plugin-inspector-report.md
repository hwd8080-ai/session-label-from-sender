...
| ------------------------- | -------- | -------------- | ----- | ----------------- | --------------------- |
| session-label-from-sender | high     | plugin-runtime | -     | definePluginEntry | gatewayMethodDispatch |
...
| ------------------------- | ----------------------- | ----- | ------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------- |
| session-label-from-sender | seam-inventory          | log   | observed 0 hooks, 1 registrations, and 1 manifest contracts                           | registration:definePluginEntry, manifestContract:gatewayMethodDispatch | -             |
| session-label-from-sender | hook-names-present      | log   | all observed hooks exist in the target OpenClaw hook registry                         | -                                                                      | -             |
| session-label-from-sender | api-registrars-present  | log   | all observed api.register* calls exist in the target OpenClaw plugin API builder      | -                                                                      | -             |
| session-label-from-sender | sdk-exports-present     | log   | all observed plugin SDK imports exist in target OpenClaw package exports              | openclaw/plugin-sdk/plugin-entry                                       | -             |
| session-label-from-sender | manifest-fields-checked | log   | plugin manifest fields were compared with target OpenClaw manifest types              | openclaw.plugin.json                                                   | -             |
| session-label-from-sender | package-metadata        | log   | selected package metadata for plugin contract checks                                  | package.json, @hwd8080-ai/session-label-from-sender, version:2026.7.2  | -             |
| session-label-from-sender | declarative-contracts   | log   | fixture declares manifest contracts that can be checked without executing plugin code | gatewayMethodDispatch                                                  | -             |
